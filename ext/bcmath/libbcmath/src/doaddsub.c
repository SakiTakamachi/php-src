...
bc_num _bc_do_add(bc_num n1, bc_num n2)
{
	...
	/* Prepare sum. */
	sum = bc_new_num_nonzeroed_with_vsize(
		MAX(n1->n_int_vsize, n2->n_int_vsize),
		MAX(n1->n_frac_vsize, n2->n_frac_vsize),
	...
	if (n1->n_int_vsize != n2->n_int_vsize) {
		...
		for (count = sum->n_int_vsize - min_int_vsize; count > 1; count--) {
			*sumptr = *n1ptr++ + carry;
			if (*sumptr >= BC_VECTOR_BOUNDARY_NUM) {
				*sumptr -= BC_VECTOR_BOUNDARY_NUM;
				carry = 1;
...
{
	ZEND_ASSERT(n1->n_int_vsize >= n2->n_int_vsize);
	...
	diff = bc_new_num_nonzeroed_with_vsize(
		n1->n_int_vsize,
		MAX(n1->n_frac_vsize, n2->n_frac_vsize),
		MAX(n1->n_scale, n2->n_scale)
	);
